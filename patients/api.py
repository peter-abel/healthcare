...
class PatientViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        for doctors, or just the patient for the patient user.
        """
...
class MedicalRecordViewSet(viewsets.ModelViewSet):
    ...
    filter_backends = [filters.SearchFilter, filters.OrderingFilter]
    search_fields = ['diagnosis', 'treatment', 'medications', 'notes']
    ...
    def get_queryset(self):
        """
        This view should return a list of all medical records
        for the currently authenticated user.
        """
        user = self.request.user
        
        # Cache key based on user ID and query parameters
        cache_key = f'medical_records_user_{user.id}_{self.request.query_params}'
        
        # Define the query function
        def get_medical_records():
            if hasattr(user, 'patient'):
                # User is a patient, return their medical records
                return MedicalRecord.objects.filter(patient=user.patient)
            elif hasattr(user, 'doctor'):
                # User is a doctor, return medical records they created
        ...
        # Get cached data or execute query
        return get_cached_data(cache_key, settings.PATIENT_CACHE_TIMEOUT, get_medical_records)
    ...
    def perform_create(self, serializer):
        ...
        # Save the medical record with the doctor
        medical_record = serializer.save(doctor=doctor)
        
        # Invalidate cache
        invalidate_cache_pattern(f'medical_records_user_{self.request.user.id}_*')
        invalidate_cache_pattern(f'medical_records_patient_{medical_record.patient.id}')
        invalidate_cache_pattern(f'medical_records_user_{medical_record.patient.user.id}_*')
    ...
    def perform_destroy(self, instance):
        ...
        # Invalidate cache
        invalidate_cache_pattern(f'medical_records_user_{doctor_user_id}_*')
        invalidate_cache_pattern(f'medical_records_patient_{patient_id}')
        invalidate_cache_pattern(f'medical_records_user_{patient_user_id}_*')
